...
schema_type_t uvisits_type_arr[] = {TINYTEXT, TINYTEXT, INTEGER,
                                    INTEGER,  TINYTEXT, TINYTEXT,
                                    TINYTEXT, TINYTEXT, INTEGER};
...
int populate_database_from_csv(std::string fname, int num_rows, int db_id,
                               int table_id, schema_t *sc,
...
void column_sort_table_parallel(sgx_enclave_id_t eid, int db_id, int table_id,
                                int field, int num_threads);
...
void thread_fn(sgx_enclave_id_t eid) { ecall_null_ecall(eid); }

int test_threads(sgx_enclave_id_t eid) {
    printf(TXT_FG_YELLOW "Simple threads test" TXT_NORMAL
                         ": Creating threads\n");
...
void bcache_read_write_fn(sgx_enclave_id_t eid, int db_id, int from_table_id,
                          int to_table_id) {
    int ret;
    sgx_status_t sgx_ret = SGX_ERROR_UNEXPECTED;

    sgx_ret = ecall_bcache_test_read_write(eid, &ret, db_id, from_table_id,
                                           to_table_id);
    if (sgx_ret || ret) {
...
void bcache_test_run_threads(sgx_enclave_id_t eid, int db_id, int from_table_id,
                             int to_table_id) {
...
int bcache_test(sgx_enclave_id_t eid, int db_id, int from_table_id) {
    ...
    sgx_ret = ecall_bcache_test_create_read_write_table(
        eid, &ret, db_id, from_table_id, &to_table_id);
    if (sgx_ret || ret) {
        ERR("error:%d (sgx ret:%d)\n", ret, sgx_ret);
        return ret;
    }
    ...
    for (int i = 0; i < 1; i++) {
        ...
        sgx_ret = ecall_bcache_test_cmp_read_write(eid, &ret, db_id,
...
void run_rankings_test(sgx_enclave_id_t eid, int db_id, int rankings_table_id) {
    ...
    printf(TXT_FG_YELLOW "Column sort test" TXT_NORMAL
                         ": sorting rankings table \n");

    unsigned long long start, end;
    start = RDTSC_START();

    column_sort_table_parallel(eid, db_id, rankings_table_id, column,
                               num_threads);
...
int test_rankings(sgx_enclave_id_t eid) {
    ...
    sgx_ret =
        ecall_create_db(eid, &ret, db_name.c_str(), db_name.length(), &db_id);
    if (sgx_ret || ret) {
        ERR("create db error:%d (sgx ret:%d)\n", ret, sgx_ret);
        return ret;
    }

    sgx_ret = ecall_create_table(eid, &ret, db_id, table_name.c_str(),
                                 table_name.length(), &sc, &rankings_table_id);
    if (sgx_ret || ret) {
        ERR("create table error:%d (sgx ret:%d)\n", ret, sgx_ret);
        goto out;
    }

    ret = populate_database_from_csv(rankings_csv, RANKINGS_TABLE_SIZE, db_id,
                                     rankings_table_id, &sc, eid);
    ...
    sgx_ret = ecall_create_table(eid, &ret, db_id, udata_table_name.c_str(),
                                 udata_table_name.length(), &sc_udata,
                                 &udata_table_id);
    if (sgx_ret || ret) {
        ERR("create table error:%d (sgx ret:%d), table:%s\n", ret, sgx_ret,
            udata_table_name.c_str());
        goto out;
    }

    ret = populate_database_from_csv(uvisits_csv, UVISITS_TABLE_SIZE, db_id,
                                     udata_table_id, &sc_udata, eid);
    ...
    /* Promote column tests */
    {
        ...
        printf(TXT_FG_YELLOW "Promote column test" TXT_NORMAL
                             ": promoting rankings table \n");

        ecall_promote_table_dbg(eid, &ret, db_id, rankings_table_id, 1,
                                &p_rankings_table_id);
        ecall_flush_table(eid, &ret, db_id, p_rankings_table_id);
        printf("promoted rankings table\n");
        ecall_print_table_dbg(eid, &ret, db_id, rankings_table_id, 359990,
                              360020);
        ecall_print_table_dbg(eid, &ret, db_id, p_rankings_table_id, 359990,
                              360020);
    }
#endif
...
#if defined(TEST_COLUMN_SORT_RANKINGS)
    {
        ...
        // ecall_column_sort_table_dbg(eid, &ret, db_id, rankings_table_id,
        // column);
        column_sort_table_parallel(eid, db_id, rankings_table_id, column,
                                   num_threads);
        ...
        printf("Sorting + flushing %d rows took %llu cycles (%f sec, %f "
               "rows/sec)\n",
               RANKINGS_TABLE_SIZE, end - start, (end - start) / cycles_per_sec,
               RANKINGS_TABLE_SIZE / ((end - start) / cycles_per_sec));
        ecall_print_table_dbg(eid, &ret, db_id, rankings_table_id, 0, 23);
    }
#endif
...
#if defined(TEST_JOIN)
    {
        ...
        printf(TXT_FG_YELLOW "Sort join test" TXT_NORMAL
                             ": joining rankings and udata tables \n");
        ...
        sgx_ret =
            ecall_hash_bin_packing_join(eid, &ret, db_id, &c, &join_table_id);
#else
        INFO("Performing external bin packing join\n");
        {
            using bin_packing_join::external_bin_packing_join::bin_packing_join;
            ret = bin_packing_join(eid, db_id, &c, rankings_csv, uvisits_csv,
                                   &join_table_id);
        }
#endif
#if defined(REPORT_BIN_PACKING_JOIN_STATS)
        auto duration_ms = std::chrono::steady_clock::now() - start;
        double secs =
            std::chrono::duration_cast<std::chrono::microseconds>(duration_ms)
                .count() /
            1E6;
        INFO("Finished: Bin-Packing-Based Merge Join takes %f seconds\n", secs);
#endif
...
void test_barrier_fn(sgx_enclave_id_t eid, unsigned long count, int num_threads,
                     int tid) {
    int ret;
...
void test_barriers(sgx_enclave_id_t eid, int num_threads, unsigned long count) {
    ...
    for (auto i = 0u; i < num_threads; i++)
        threads.push_back(
            new thread(test_barrier_fn, eid, count, num_threads, i));

    for (auto &t : threads)
        t->join();
}

void bitonic_sorter_fn(sgx_enclave_id_t eid, int db_id, int table_id, int field,
                       int tid, int num_threads) {
    int ret;
    ecall_bitonic_sort_table_parallel(eid, &ret, db_id, table_id, field, tid,
                                      num_threads);
}

void bitonic_sort_parallel(sgx_enclave_id_t eid, int db_id, int table_id,
                           int field, int num_threads) {
    std::vector<std::thread *> threads;
    assert((num_threads & (num_threads - 1)) == 0);

    for (auto i = 0u; i < num_threads; i++)
        threads.push_back(new thread(bitonic_sorter_fn, eid, db_id, table_id,
                                     field, i, num_threads));
...
int test_bitonic_sort(sgx_enclave_id_t eid) {
    ...
    sgx_ret = ecall_create_table(eid, &ret, db_id, table_name.c_str(),
                                 table_name.length(), &sc, &table_id);
    if (sgx_ret || ret) {
        ERR("create table error:%d (sgx ret:%d)\n", ret, sgx_ret);
        goto out;
    }

    ret = populate_database_from_csv(rand_csv, RANDINT_TABLE_SIZE, db_id,
                                     table_id, &sc, eid);
...
void column_sort_fn(sgx_enclave_id_t eid, int db_id, int table_id, int field,
                    int tid, int num_threads) {
    int ret;
    ecall_column_sort_table_parallel(eid, &ret, db_id, table_id, field, tid,
                                     num_threads);
}
...
void column_sort_table_parallel(sgx_enclave_id_t eid, int db_id, int table_id,
    ...
    for (auto i = 0u; i < num_threads; i++) {
        ...
        threads.push_back(new thread(column_sort_fn, eid, db_id, table_id,
                                     field, i, num_threads));
    }
    for (auto &t : threads) {
...
void quick_sorter_fn(sgx_enclave_id_t eid, int db_id, int table_id, int field,
                     int tid, int num_threads) {
    int ret;
    ecall_quicksort_table_parallel(eid, &ret, db_id, table_id, field, tid,
                                   num_threads);
}

void quick_sort_parallel(sgx_enclave_id_t eid, int db_id, int table_id,
                         int field, int num_threads) {
    std::vector<std::thread *> threads;
    assert((num_threads & (num_threads - 1)) == 0);

    for (auto i = 0u; i < num_threads; i++)
        threads.push_back(new thread(quick_sorter_fn, eid, db_id, table_id,
                                     field, i, num_threads));
...
int test_merge_sort_write(sgx_enclave_id_t eid) {
...

    {
        ...
        int num_project_columns_right =
            sizeof(project_columns_right) / sizeof(project_columns_right[0]);
        ...
        int num_pad_bytes_right =
            max(row_size(&sc), row_size(&sc_udata)) - row_size(&sc_udata);
...
#define CREATE_APPEND_TABLE
        ...
        ecall_merge_and_sort_and_write(
            eid, &ret, db_id, rankings_table_id, ptr_project_columns_left,
            num_project_columns_left, ptr_promote_columns_left,
            num_pad_bytes_left, udata_table_id, ptr_project_columns_right,
            num_project_columns_right, ptr_promote_columns_right,
            num_pad_bytes_right, &write_table_id);

        // merge_and_sort_and_write_parallel(eid, db_id, table_id, 0,
        // num_threads);
        ...
        printf(
            "merge sort write table (in-place) + flushing took %llu cycles\n",
            end - start);
